...
const Aupsc = (props)=>{
    ...
    <View style={{ marginBottom:15, justifyContent:'center', alignItems:'center', backgroundColor:'#df0d55'  }}>
      <Text style={AupscStyle.mainTitle}>{data?.title}</Text>
    </View>  
       ...
       {Object.entries(data?.exams).map(([key, val], index)=>{
         return (<View key={index} style={{ paddingTop: 15 }}>
            <View style={{ flex:1, flexDirection:'row' }}>
              <Text style={[AupscStyle.title, { width:'8%' }]}>{index>=9?(index+1):'0'+(index+1)}.</Text>
               <View style={{ width:'92%' }}>
...
const AupscStyle = StyleSheet.create({ 
   ...
   //  mainTitle: { letterSpacing:0.8, fontSize:18, paddingBottom:6, fontWeight:'bold', color:'#000', lineHeight: 22 },
    title: { fontSize:16, paddingBottom:6, fontWeight:'bold', color:'#000', lineHeight: 22 },
    desc: { color:'#333', lineHeight: 22 },
    list:{ paddingTop:2, color:'#333', lineHeight: 22 }
 });
